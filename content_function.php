...
function dispsubcategories($parent_id) {
	...
	while ($row = mysqli_fetch_assoc($select)) {
		echo "<tr><td class='category_title'><a href='/forum/topics.php?cat_id=".$row['cat_id']."&subcat_id=".$row['subcat_id']."'>
...
function disptopics($cid, $scid) {
	...
	if (!$select || mysqli_num_rows($select) !=1) {
		...
		while ($row = mysqli_fetch_row($select)) {  //hier is een fout ik heb de oplossing niet kunnen vinden
